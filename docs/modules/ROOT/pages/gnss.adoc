...
[stem]
++++
r^a_s = \left\| C^e_I(t_{st})\, r^e_{es}(t_{st}) - r^e_{ea}(t_{sa}) \right\|
\qquad
u^e_{as} = \frac{C^e_I(t_{st})\, r^e_{es}(t_{st}) - r^e_{ea}(t_{sa})}{r^a_s}
\qquad \text{(8.35), (8.41)}
++++
...
[stem]
++++
\hat{x} = \hat{x}^- + \left(H^T H\right)^{-1} H^T \, \delta z
++++

where stem:[\delta z] is the vector of measured-minus-predicted pseudo-ranges (or rates). The solve repeats until
...
with the system noise covariance stem:[Q_{k-1}] built from the acceleration and clock-noise power spectral
