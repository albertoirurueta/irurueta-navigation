...
public abstract class RobustWifiAccessPointPowerAndPositionEstimator3D extends
     ...
     */
    protected RssiRadioSourceEstimator3D mInnerEstimator =
            new RssiRadioSourceEstimator3D();

    /**
     * Subset of readings used by inner estimator.
     */
    private List<RssiReadingLocated<WifiAccessPoint, Point3D>> mInnerReadings = new ArrayList<>();
    ...
    public WifiAccessPointWithPowerAndLocated3D getEstimatedAccessPoint() {
        List<? extends RssiReadingLocated<WifiAccessPoint, Point3D>> readings = getReadings();
        if (readings == null || readings.isEmpty()) {
            return null;
        }
        WifiAccessPoint accessPoint = readings.get(0).getSource();
